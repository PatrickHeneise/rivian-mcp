...
export async function validateOtp(email, otpCode) {
  ...
  if (!data?.loginWithOTP?.accessToken) {
    throw new Error('OTP validation failed — incorrect code or expired token. Please try again.')
  }
...
async function gql(url, body, extraHeaders = {}) {
  ...
  try {
    ...
    throw new Error(`Network error contacting Rivian API: ${err.message}`)
  }
